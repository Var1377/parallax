...
use crate::inet::reductions::{remove_node, annihilate_any, get_partition_ptr_mut, get_aux_ports, connect, add_redex_to_partition};
use parallax_net::{node::{NodeType, Static}, port::{Port, PortType}, Redex};
...
pub(crate) unsafe fn reduce_static(
    ...
    let (s_port, other_port) = if NodeType::from_u8(port_a.node_type()) == Some(NodeType::Static) {
        ...
        log::error!("reduce_static called with no Static node in redex: {:?}", redex);
        return;
    };

    let s_data = get_static_data(s_port, read_guard);
    let s_tag = decode_tag(s_data);
    let s_payload = decode_data(s_data);
    ...
    match other_type {
        ...
        Some(NodeType::Constructor) => {
            ...
            match s_tag {
                TAG_FUNCTION => {
                    ...
                    // This handles both function calls and constructor tags
                    static_call_generic(s_port, s_payload, other_port, worker, read_guard, compiled_defs);
                }
                TAG_INTRINSIC_OP => {
                    // Static(Intrinsic|OpCode) ~ Constructor(AppHead, Arg)
                    dispatch_intrinsic_op(redex, s_port, s_payload, other_port, worker, read_guard);
                }
                TAG_IS_VARIANT => {
                    // Static(IsVariant|VariantSym) ~ Constructor(Tag, Payload)
                    // Determine the caller (the port connected to Static(IsVariant))
                    let caller_port = *get_port_ptr_mut(read_guard, s_port).expect("IsVariant port not found");
                    static_is_variant_op(s_port, s_data, other_port, caller_port, read_guard);
                }
                TAG_NIL => {
        ...
        Some(NodeType::Duplicator) => {
            // Static ~ Duplicator -> Duplicate the Static node
            ...
            // Use the existing data for the new static node
            let new_static_idx = target_partition.alloc_static(Static {
                principle: Port::NULL,
                data: s_data.into(), // Create new AtomicU64 from existing data
            });
            let new_static_port = Port::principal(NodeType::Static, target_partition_id, new_static_idx as u64);
            target_partition.statics[new_static_idx].principle = new_static_port;

            // Get the two aux ports from the duplicator
            ...
            if dup_left.port_type() == PortType::Principal { add_redex_to_partition(target_partition_id, Redex(dup_left, s_port), read_guard); }
            if dup_right.port_type() == PortType::Principal { add_redex_to_partition(target_partition_id, Redex(dup_right, new_static_port), read_guard); }

            remove_node(other_port, read_guard); // Remove the Duplicator
        }
