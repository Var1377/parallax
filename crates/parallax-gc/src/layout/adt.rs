...
impl<'a> LayoutComputer<'a> {
    ...
    pub(crate) fn compute_struct_layout(&mut self, struct_def: &HirStructDef) -> Result<LayoutDescriptor, LayoutError> {
        ...
        for (offset, field_desc_idx) in &fields_with_offsets {
            ...
            // Cannot borrow self.descriptors mutably here, need immutable access.
            // Assuming LayoutComputer gets an immutable ref to the descriptors vec
            // for lookups, or we pass it explicitly.
            // Let's assume immutable lookup is possible:
            let field_descriptor = self.descriptors.get(*field_desc_idx)
                .ok_or(LayoutError::MissingDescriptor(*field_desc_idx))?;
            
            // Check if the field descriptor contains handles (requires immutable self.descriptors)
    ...
    /// Converts an HirType to a repc Type for layout computation.
    pub(crate) fn hir_type_to_repc_type(&mut self, hir_type: &HirType) -> Result<Type<()>, LayoutError> {
        match hir_type {
            ...
            // Other types will be implemented in the next parts...
            _ => {
                // For now, handle other types as pointers
                Ok(Type {
                    layout: (),
                    annotations: vec![],
                    variant: TypeVariant::Builtin(BuiltinType::Pointer),
