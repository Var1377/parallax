...
use parallax_ir::ast::{self, Operator};
...
fn remove_variable_nodes(nodes: &mut Vec<Node<'_>>) {
...
impl<'a> From<ast::NamedNetwork<'a>> for NamedNet<'a> {
    fn from(ast: ast::NamedNetwork<'a>) -> Self {
        ...
        let entry_point = traverse(ast.net.tree, &mut ctx);
        ...
        remove_variable_nodes(&mut nodes);

        NamedNet { name: ast.name, nodes, entry_point, redexes }
    }
}

impl<'a> From<ast::Book<'a>> for IRBook<'a> {
    fn from(ast: ast::Book<'a>) -> Self {
        IRBook { named_nets: ast.named_nets.into_iter().map(|net| net.into()).collect() }
    }
}
...
mod tests {
    use super::*;
    use parallax_ir::ast::{self, Book, NamedNetwork, Network, Tree, Numeric as AstNumeric, Operation as AstOperation};
