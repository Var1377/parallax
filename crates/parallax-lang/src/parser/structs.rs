...
pub fn parse_struct_body(node: &Node, source: &str) -> Result<Vec<StructField>, ParallaxError> {
    println!("Parsing struct body: {}", node.to_sexp());
...
fn parse_struct_field(node: &Node, source: &str) -> Result<StructField, ParallaxError> {
    println!("Parsing struct field: {}", node.to_sexp());
...
pub fn parse_struct_def(node: &Node, source: &str) -> Result<StructDef, ParallaxError> {
    println!("Parsing struct definition: {}", node.to_sexp());
...
pub fn parse_tuple_body(node: &Node, source: &str) -> Result<Vec<crate::ast::types::Type>, ParallaxError> {
    println!("Parsing tuple body: {}", node.to_sexp());
...
mod tests {
    ...
    #[test]
    fn test_empty_struct_body() -> Result<(), ParallaxError> {
    ...
    #[test]
    fn test_struct_body_with_fields() -> Result<(), ParallaxError> {
    ...
    #[test]
    fn test_struct_body_with_visibility() -> Result<(), ParallaxError> {
        ...
        let fields = parse_struct_body(&body_node, source)?;
        assert_eq!(fields.len(), 2);
        assert_eq!(fields[0].name.0, "name");
        assert!(!fields[0].visibility);
        assert_eq!(fields[1].name.0, "age");
