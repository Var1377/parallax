...
use crate::context::inference::TypeEnvironment; // Updated path
use crate::types::{PrimitiveType, Ty, TyKind, TypedExpr, TypedExprKind, TypedMatchArm, TypedPattern, TypedPatternKind};
...
pub(crate) fn check_block(
    ...
    let original_env = checker._type_env.clone(); // Renamed
    // Correctly create a mutable binding for block_env
    let block_env_inner = TypeEnvironment::with_parent(original_env.clone());
    // Shadow checker.type_env correctly
    checker._type_env = Arc::new(block_env_inner); // Assign the new Arc // Renamed
    ...
    for (i, expr) in exprs.iter().enumerate() {
        ...
        if let ResolvedExprKind::Let { pattern, value, type_annotation } = &expr.kind {
            ...
            // Placeholder - remove when check_pattern is uncommented
            //  let typed_pattern = TypedPattern {
            //      kind: TypedPatternKind::Wildcard, // Placeholder
            //      ty: value_ty.clone(),             // Placeholder
    ...
    checker._type_env = original_env; // Renamed
...
pub(crate) fn add_pattern_bindings(checker: &mut TypeChecker, pattern: &TypedPattern) -> TypeResult<()> {
    match &pattern.kind {
        TypedPatternKind::Identifier { symbol, name } => {
            ...
            if let Some(current_env) = Arc::get_mut(&mut checker._type_env) {
        ...
        TypedPatternKind::Struct { fields, .. } => {
         ...
         TypedPatternKind::Or(_left, _right) => {
...
pub(crate) fn check_match(
    ...
    for (pattern, body) in arms {
        ...
        let arm_env_inner = TypeEnvironment::with_parent(original_env.clone()); // Removed mut
        checker._type_env = Arc::new(arm_env_inner); // Shadow checker.type_env // Renamed
        ...
        // let typed_pattern = TypedPattern {
        //     kind: TypedPatternKind::Wildcard, // Placeholder
        //     ty: scrutinee_ty.clone(),        // Placeholder
        //     span: pattern.span,            // Placeholder
        // };
