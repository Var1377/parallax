...
use parallax_types::types::{Ty, TypedParameter, TypedFunction, TypedExpr, TypedExprKind, TyKind, PrimitiveType, TypedPattern, TypedPatternKind, TypedStruct, TypedField, TypedEnum, TypedVariant, TypedArgument, TypedDefinitions, TypedModule};
...
fn test_inline_simple_call() {
    ...
    functions.insert(callee_sym, TypedFunction {
        name: "small_callee".to_string(), params: vec![], return_type: ty_i32(),
    ...
    functions.insert(main_sym, TypedFunction {
        name: "main".to_string(), params: vec![], return_type: ty_i32(),
        body: Some(TypedExpr {
            kind: TypedExprKind::Call { func: Box::new(TypedExpr { kind: TypedExprKind::Variable { symbol: callee_sym, name: "small_callee".to_string() }, ty: fn_type, span: dummy_span() }), args: vec![] },
    ...
    let original_node_count_main = count_nodes(find_fn(&hir_module, "main").unwrap().body.as_ref().unwrap());

    perform_inlining(&mut hir_module);

    let main_fn = find_fn(&hir_module, "main").expect("Main function missing after inlining");
    let main_body = main_fn.body.as_ref().expect("Main body missing after inlining");
    let (bindings, tail) = flatten_hir_expr(main_body.clone());

    match tail {
        HirTailExpr::Return(Operand::Const(HirLiteral::Int(1))) if bindings.is_empty() => { /* OK: Directly returned */ }
        HirTailExpr::Return(Operand::Var(ret_var)) => {
            assert_eq!(bindings.len(), 1, "Expected one binding for the inlined literal");
            let (var, ty, val) = &bindings[0];
            assert_eq!(*var, ret_var);
            assert_eq!(*ty, HirType::Primitive(ResolvePrimitiveType::I32));
            assert_eq!(*val, HirValue::Use(Operand::Const(HirLiteral::Int(1))));
        }
        _ => panic!("Unexpected tail expression in main after inlining: {:?}", tail),
    }
}
...
fn test_no_inline_recursive() {
    ...
    let recurse_params = vec![TypedParameter { name: "n".to_string(), symbol: param_n_sym, ty: ty_i32(), is_variadic: false, has_default: false, span: dummy_span() }];
    let recurse_body_ast = TypedExpr { kind: TypedExprKind::Literal(parallax_syntax::ast::common::Literal::Int(0)), ty: ty_i32(), span: dummy_span() }; 
    functions.insert(recurse_sym, TypedFunction { name: "recurse".to_string(), params: recurse_params.clone(), return_type: ty_i32(), body: Some(recurse_body_ast), generic_params: vec![], span: dummy_span(), is_effectful: false });
    ...
    let recurse_fn_hir = hir_module.functions.iter_mut().find(|f| f.symbol == recurse_sym).unwrap();
    let recurse_call_val = HirValue::Call { func: Operand::Global(recurse_sym), args: vec![Operand::Const(HirLiteral::Int(0))] };
    let next_var_id = hir_module.next_var_id;
    let binding_var = HirVar(next_var_id);
    hir_module.next_var_id += 1;
    let tail_return = HirExpr { kind: HirExprKind::Tail(HirTailExpr::Return(Operand::Var(binding_var))), ty: HirType::Primitive(ResolvePrimitiveType::I32), span: dummy_span() };
    let recurse_call_expr = HirExpr { kind: HirExprKind::Let { var: binding_var, var_ty: HirType::Primitive(ResolvePrimitiveType::I32), value: Box::new(recurse_call_val), rest: Box::new(tail_return) }, ty: HirType::Primitive(ResolvePrimitiveType::I32), span: dummy_span() };
    ...
    assert_eq!(main_body_before, main_body_after, "Recursive function should not be inlined");
}
...
fn test_no_inline_large_function() {
    ...
    for i in 0..50 { // Assuming threshold is around 25
        let let_binding = TypedExpr {
            kind: TypedExprKind::Let { 
                pattern: TypedPattern { kind: TypedPatternKind::Identifier { symbol: current_var_sym, name: format!("v{}", i)}, ty: ty_i32(), span: dummy_span() }, 
                value: Box::new(TypedExpr { kind: TypedExprKind::Literal(parallax_syntax::ast::common::Literal::Int(i)), ty: ty_i32(), span: dummy_span()}) // Simplified value
            },
            ty: ty_unit(), 
            span: dummy_span(),
        };
    ...
    large_body_exprs.push(TypedExpr { kind: TypedExprKind::Variable { symbol: prev_var_sym, name: format!("v{}", 49)}, ty: ty_i32(), span: dummy_span()});
    let large_body = TypedExpr { kind: TypedExprKind::Block(large_body_exprs), ty: ty_i32(), span: dummy_span() };

    functions.insert(callee_sym, TypedFunction { name: "large_callee".to_string(), params: vec![], return_type: ty_i32(), body: Some(large_body), generic_params: vec![], span: dummy_span(), is_effectful: false });
    functions.insert(main_sym, TypedFunction {
        name: "main".to_string(), params: vec![], return_type: ty_i32(),
        body: Some(TypedExpr { kind: TypedExprKind::Call { func: Box::new(TypedExpr { kind: TypedExprKind::Variable { symbol: callee_sym, name: "large_callee".to_string() }, ty: fn_type, span: dummy_span() }), args: vec![] }, ty: ty_i32(), span: dummy_span(), }),
        generic_params: vec![], span: dummy_span(), is_effectful: false,
    });
    ...
    assert_eq!(main_body_before, main_body_after, "Large function should not be inlined");
}
...
fn test_no_inline_mutually_recursive() {
    ...
    assert_eq!(main_body_before, main_body_after, "Mutually recursive functions should not be inlined");
}
...
fn test_inline_multiple_calls() {
    ...
    functions.insert(callee_sym, TypedFunction { name: "callee".to_string(), params: vec![], return_type: ty_i32(), body: Some(TypedExpr { kind: TypedExprKind::Literal(parallax_syntax::ast::common::Literal::Int(1)), ty: ty_i32(), span: dummy_span() }), generic_params: vec![], span: dummy_span(), is_effectful: false });
    
    let call1 = TypedExpr { kind: TypedExprKind::Call { func: Box::new(TypedExpr { kind: TypedExprKind::Variable { symbol: callee_sym, name: "callee".to_string() }, ty: fn_type.clone(), span: dummy_span() }), args: vec![] }, ty: ty_i32(), span: dummy_span() };
    let let1 = TypedExpr { kind: TypedExprKind::Let { pattern: TypedPattern { kind: TypedPatternKind::Identifier{ symbol: var_c1, name: "c1".to_string() }, ty: ty_i32(), span: dummy_span()}, value: Box::new(call1)}, ty: ty_unit(), span: dummy_span()};
    let call2 = TypedExpr { kind: TypedExprKind::Call { func: Box::new(TypedExpr { kind: TypedExprKind::Variable { symbol: callee_sym, name: "callee".to_string() }, ty: fn_type.clone(), span: dummy_span() }), args: vec![] }, ty: ty_i32(), span: dummy_span() };
    // Simplified return, just return second call result
    let main_body = TypedExpr { kind: TypedExprKind::Block(vec![let1, call2]), ty: ty_i32(), span: dummy_span() }; 
    ...
    assert_eq!(bindings.len(), 3, "Expected three bindings after inlining (including the unused one)");

    // Check binding structure (reversed order)
    // bindings[0]: let v1 = 1 (from second inlined call)
    assert!(matches!(bindings[0].2, HirValue::Use(Operand::Const(HirLiteral::Int(1)))), "Binding 0 (v1) should be inlined constant 1");
    let v1 = bindings[0].0; // Get the HirVar for v1

    // bindings[2]: let v0 = 1 (from first inlined call)
    assert!(matches!(bindings[2].2, HirValue::Use(Operand::Const(HirLiteral::Int(1)))), "Binding 2 (v0) should be inlined constant 1");
    let v0 = bindings[2].0; // Get the HirVar for v0

    // bindings[1]: let v_c1 = v0 (assignment of unused var_c1)
    assert!(matches!(bindings[1].2, HirValue::Use(Operand::Var(var)) if var == v0), "Binding 1 (v_c1) should be assignment from first call's result (v0)");

    // Check tail returns the variable from the second inlined call's result binding (v1)
    assert!(matches!(tail, HirTailExpr::Return(Operand::Var(ret_var)) if ret_var == v1), "Tail should return result of second call (v1)");
}
