...
/// </call_expr>
pub fn parse_call_expr<'a>(node: &Node<'a>, source: &'a str) -> Result<Expr, SyntaxError> {
    let span = create_span(node);
    
    // The first child is the function being called
    let callee_node = node.child(0).ok_or_else(|| SyntaxError::ParseError {
    ...
    let args_node = node.child(1).ok_or_else(|| SyntaxError::ParseError {
...
///   ")"
/// </call_args>
pub fn parse_call_args<'a>(node: &Node<'a>, source: &'a str) -> Result<Vec<Argument>, SyntaxError> {
...
/// </argument>
fn parse_argument<'a>(node: &Node<'a>, source: &'a str) -> Result<Argument, SyntaxError> {
    let span = create_span(node);
    
    // Check if this is a spread argument (starts with ...)
    if node.kind() == "spread_element" {
        let expr_node = node.child(1).ok_or_else(|| SyntaxError::ParseError {
        ...
        return Ok(Argument {
            name: Some(Ident {
                name: "...".to_string(),
                span: node.parent().map_or(span, |p| create_span(&p)),
            }),
            value: expr,
            span,
        });
    ...
    if node.kind() == "named_argument" {
        let name_node = node.child(0).ok_or_else(|| SyntaxError::ParseError {
            message: format!("Named argument missing name at {:?}", span),
            span: Some(span.clone()),
        })?;
        
        let value_node = node.child(2).ok_or_else(|| SyntaxError::ParseError {
...
mod tests {
    ...
    #[test]
    fn test_simple_call() -> Result<(), SyntaxError> {
    ...
    #[test]
    fn test_empty_call() -> Result<(), SyntaxError> {
        // Put the call in a proper function context
    ...
    #[test]
    fn test_multiple_args() -> Result<(), SyntaxError> {
        // Put the call in a proper function context
    ...
    #[test]
    fn test_named_args() -> Result<(), SyntaxError> {
        // Put the call in a proper function context
    ...
    #[test]
    fn test_spread_arg() -> Result<(), SyntaxError> {
        // Put the call in a proper function context
